...
import { fetchFlaxNutritionData, fetchFlaxRegions, fetchFlaxVarieties } from "../../../apis/data_apis";
...
export function HplcDatabaseSection() {
  ...
  useEffect(() => {
    Promise.all([
      fetchFlaxRegions({ limit: 500 }),
      fetchFlaxVarieties({ limit: 500 }),
  ...
  useEffect(() => {
    setLoading(true);
    fetchFlaxNutritionData({ page, pageSize, ...filters })
      .then((data) => {
  ...
  useEffect(() => {
    setPage(1);
    fetchFlaxNutritionData({ limit: 5000, ...filters })
      .then((data) => {
