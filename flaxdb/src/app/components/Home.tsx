...
export function Home() {
  ...
  return (
    <div className="bg-[#f8faf5]">
      <section className="border-b border-blue-100 bg-gradient-to-br from-blue-50 via-white to-sky-50">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-16">
          <div className="flex flex-col justify-center">
            <div className="mb-5 inline-flex w-fit items-center gap-2 rounded-full border border-blue-200 bg-white px-4 py-2 text-sm font-medium text-blue-900 shadow-sm">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                to="/data"
                className="inline-flex items-center rounded-xl bg-blue-500 px-6 py-3 font-medium text-white shadow-lg shadow-blue-200 transition-colors hover:bg-blue-700"
              >
                {t("home.explore")}
              ...
              <Link
                to="/tools"
                className="inline-flex items-center rounded-xl border border-blue-200 bg-white px-6 py-3 font-medium text-blue-900 transition-colors hover:bg-blue-50"
              >
          ...
          <div className="self-center">
            <div className="relative overflow-hidden rounded-[2rem] border border-white bg-white p-3 shadow-2xl shadow-blue-100">
            ...
            <div className="-mt-6 mx-4 grid grid-cols-2 gap-3 rounded-2xl border border-blue-100 bg-white/95 p-4 shadow-xl backdrop-blur">
              {dataOverview.map((item) => (
                <div key={item.label} className="rounded-xl bg-blue-50/80 p-4">
                  <div className="text-2xl font-bold text-slate-950">{item.value}</div>
                  <div className="mt-1 text-sm font-medium text-blue-900">{item.label}</div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-blue-700">{t("home.resourcesEyebrow")}</p>
            <h2 className="mt-2 text-3xl font-bold text-slate-950">{t("home.resourcesTitle")}</h2>
            <p className="mt-3 max-w-3xl text-slate-600">
              {t("home.resourcesDesc")}
            </p>
          </div>
          <Link to="/data" className="inline-flex items-center text-sm font-semibold text-blue-800 hover:text-blue-900">
            {t("home.browseDatasets")} <ArrowRight className="ml-1 h-4 w-4" />
          </Link>
        </div>

        <div className="grid gap-5 md:grid-cols-3">
          <Link to="/data" className="group rounded-2xl border border-blue-100 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
            <Database className="mb-5 h-11 w-11 text-blue-500" />
            <h3 className="text-xl font-semibold text-slate-950 group-hover:text-blue-800">{t("home.genomicData")}</h3>
            <p className="mt-3 text-sm leading-6 text-slate-600">{t("home.genomicDataDesc")}</p>
          </Link>
          <Link to="/tools" className="group rounded-2xl border border-blue-100 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
            <Search className="mb-5 h-11 w-11 text-blue-500" />
            <h3 className="text-xl font-semibold text-slate-950 group-hover:text-blue-800">{t("home.analysisTools")}</h3>
            <p className="mt-3 text-sm leading-6 text-slate-600">{t("home.analysisToolsDesc")}</p>
          </Link>
          <Link to="/events" className="group rounded-2xl border border-blue-100 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
            <Users className="mb-5 h-11 w-11 text-blue-500" />
            <h3 className="text-xl font-semibold text-slate-950 group-hover:text-blue-800">{t("home.varieties")}</h3>
          ...
          <section className="mx-auto max-w-7xl px-4 pb-12 pt-8 sm:px-6 lg:px-8">
            <div className="grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(0,1fr)] lg:items-stretch">
              <aside className="rounded-2xl bg-slate-950 p-6 text-white shadow-xl">
                <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-500 text-slate-950">
                  <Megaphone className="h-6 w-6" />
                </div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-200">{t("home.scrolling.eyebrow")}</p>
                ...
                <div className="mt-6 grid grid-cols-2 gap-3">
                  <div className="rounded-2xl bg-white/10 p-4">
                    ...
                    <div className="mt-1 text-xs text-blue-100">{t("home.scrolling.activeNotices")}</div>
                  </div>
                  <div className="rounded-2xl bg-white/10 p-4">
                    <div className="text-3xl font-bold">{String(currentNewsIndex + 1).padStart(2, "0")}</div>
                    <div className="mt-1 text-xs text-blue-100">{t("home.scrolling.selectedItem")}</div>
                  </div>
                </div>
              </aside>
              <article className="overflow-hidden rounded-2xl border border-blue-100 bg-white shadow-sm">
                <div className="grid h-full gap-0 lg:grid-cols-[minmax(0,1fr)_180px]">
                  ...
                  <div className="border-t border-blue-100 bg-blue-50/70 p-5 lg:border-l lg:border-t-0">
                    <div className="text-xs font-semibold uppercase tracking-[0.14em] text-blue-700">{t("home.scrolling.currentItem")}</div>
          ...
          <div className="mt-6 grid gap-3">
            {workflowSteps.map((step, index) => (
              <div key={step} className="flex items-center gap-3 rounded-2xl bg-white/8 p-3">
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-blue-500 text-sm font-bold text-slate-950">
                  {index + 1}
                </span>
          ...
          {researchModules.map((module) => {
            ...
            return (
              <div key={module.title} className="rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-sm">
                <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-blue-100 text-blue-800">
      ...
      <section className="border-y border-blue-100 bg-white py-14">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:px-8">
          <div className="relative overflow-hidden rounded-[1.75rem]">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-100">{t("home.context.eyebrow")}</p>
              <h2 className="mt-2 text-3xl font-bold">{t("home.context.title")}</h2>
            </div>
          </div>

          <div className="flex flex-col justify-center">
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-blue-700">{t("home.context.signalEyebrow")}</p>
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {contextTags.map((tag) => (
                <span key={tag} className="rounded-full bg-blue-50 px-3 py-1 text-sm font-medium text-blue-800 ring-1 ring-blue-100">
                  {tag}
                </span>
              ))}
            </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-blue-700">{t("home.releaseNotes")}</p>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {changelog.map((item) => (
              <Link
                ...
                className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all duration-300 hover:border-blue-300 hover:shadow-lg"
              >
                <div className="mb-3 flex items-center justify-between gap-3">
                  <div className="text-sm text-blue-700">{formatDate(item.release_date)}</div>
                  <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-900">
                    v{item.version}
                  </span>
                </div>
                <h3 className="text-xl font-semibold text-slate-950 hover:text-blue-700">{cleanText(item.title, t("home.releaseFallbackTitle"))}</h3>
