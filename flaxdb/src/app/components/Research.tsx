...
export function Research() {
  ...
  return (
    <div className="space-y-6 bg-white p-0 text-slate-900">
      <section className="overflow-hidden bg-[radial-gradient(circle_at_top_left,#dbeafe,transparent_34%),linear-gradient(135deg,#eff6ff,#ecfeff)] border border-blue-100 p-6 shadow-sm border-l-4 border-blue-500">
        <div className="grid gap-6 xl:grid-cols-[1fr_auto] xl:items-end">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-blue-700">{t("research.eyebrow")}</p>
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_auto]">
        ...
        <div className="flex flex-wrap gap-2">
          {["nutrition", "varieties", "genes", "regions"].map((item) => (
            <a key={item} href={`/api/export/${item}/`} className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 px-3 py-2 text-sm font-medium text-white transition-colors">
        ...
        <Panel title={t("research.sections.nutritionMatrix")} icon={<Table2 className="h-5 w-5" />}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-[0.12em] text-blue-700">
                <tr>
        ...
        <Panel title={t("research.sections.nutritionChart")} icon={<BarChart3 className="h-5 w-5" />}>
          <div className="space-y-4">
            {topNutrition.map((row, index) => {
              ...
              return <div key={`bar-${row.id}-${index}`}><div className="mb-1 flex justify-between gap-4 text-xs text-slate-500"><span className="truncate">{cleanText(row.variety_code || row.variety_name || row.sample_code, varietyFallback)}</span><span>{row.oil_content ?? 0}{t("research.units.percent")}</span></div><div className="h-2.5 bg-slate-100"><div className="h-2.5 bg-blue-500" style={{ width: `${width}%` }} /></div></div>;
            })}
          </div>
        </Panel>
      </section>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {regionVisuals.map((row, index) => (
              <div key={`${row.id}-${index}`} className="border border-slate-100 bg-white p-3 shadow-sm">
                <div className="flex items-center justify-between gap-3"><span className="truncate font-medium text-slate-900">{cleanText(row.code || row.name, regionFallback)}</span><span className="text-xs text-blue-700">{row.variety_count || 0} {t("research.units.varieties")}</span></div>
                <div className="mt-2 h-2 bg-slate-100"><div className="h-2 bg-blue-500" style={{ width: `${Math.min(100, 12 + numberValue(row.variety_count) * 12)}%` }} /></div>
          ...
          <Panel title={t("research.sections.expressionHeat")} icon={<Dna className="h-5 w-5" />}>
            <div className="grid grid-cols-10 gap-1">
              {heatRows.map((row, index) => (
                <div key={`${row.id}-${index}`} title={`${cleanText(row.gene_id || row.gene, geneFallback)} ${row.expression_value || row.tpm || 0}`} className="aspect-square bg-cyan-50 ring-cyan-100 ring-1" style={{ opacity: Math.min(1, 0.25 + numberValue(row.expression_value || row.tpm || row.fpkm) / 80) }} />
              ))}
            </div>
          </Panel>
          ...
          <Panel title={t("research.sections.proteinNetwork")} icon={<Network className="h-5 w-5" />}>
            <div className="relative min-h-[260px] overflow-hidden bg-slate-50">
              {network.map((edge, index) => (
                <div key={`${edge.source}-${index}`} className="absolute flex h-16 w-16 items-center justify-center rounded-full border border-white bg-cyan-50 ring-cyan-100 p-2 text-center text-[10px] font-medium text-blue-700 shadow-sm" style={{ left: `${8 + (index % 4) * 23}%`, top: `${8 + Math.floor(index / 4) * 15}%` }}>
...
function Stat({ icon, label, value }: { icon: ReactNode; label: string; value: number }) {
  return <div className="border-l-4 border-blue-500 bg-white p-3 shadow-sm"><div className="flex items-center gap-2 text-blue-700">{icon}<span className="text-xs">{label}</span></div><div className="mt-2 text-2xl font-bold text-slate-950">{value}</div></div>;
}

function Panel({ title, icon, children }: { title: string; icon: ReactNode; children: ReactNode }) {
  return <section className="border-l-4 border-blue-500 bg-white p-5 shadow-sm"><div className="mb-4 flex items-center gap-2 text-blue-700">{icon}<h2 className="text-lg font-semibold text-slate-950">{title}</h2></div>{children}</section>;
}