...
import {
  fetchPerillaNews,
  fetchPerillaChangelogs,
  fetchPerillaRegions,
  fetchPerillaVarieties,
  fetchPerillaGenes,
  fetchPerillaGeneExpressions,
  fetchPerillaEnvironmentalFactors,
  fetchPerillaInstitutions,
  fetchPerillaAnnouncements,
  fetchPerillaDownloadFiles,
  createPerillaNews,
  updatePerillaNews,
  deletePerillaNews,
  createPerillaChangelog,
  updatePerillaChangelog,
  deletePerillaChangelog,
  createPerillaRegion,
  updatePerillaRegion,
  deletePerillaRegion,
  createPerillaVariety,
  updatePerillaVariety,
  deletePerillaVariety,
  createPerillaGene,
  updatePerillaGene,
  deletePerillaGene,
  createPerillaGeneExpression,
  updatePerillaGeneExpression,
  deletePerillaGeneExpression,
  createPerillaEnvironmentalFactor,
  updatePerillaEnvironmentalFactor,
  deletePerillaEnvironmentalFactor,
  createPerillaInstitution,
  updatePerillaInstitution,
  deletePerillaInstitution,
  createPerillaAnnouncement,
  updatePerillaAnnouncement,
  deletePerillaAnnouncement,
  createPerillaDownloadFile,
  updatePerillaDownloadFile,
  deletePerillaDownloadFile,
  fetchPerillaNutritionData,
  createPerillaNutritionData,
  updatePerillaNutritionData,
  deletePerillaNutritionData,
  fetchPerillaRegionalMapSites,
  createPerillaRegionalMapSite,
  updatePerillaRegionalMapSite,
  deletePerillaRegionalMapSite,
  fetchPerillaRegionalEnvironmentValues,
  createPerillaRegionalEnvironmentValue,
  updatePerillaRegionalEnvironmentValue,
...
const dataTypeConfig: Record<DataType, { title: string; icon: React.ElementType; fetchFn: (params?: any) => Promise<any>; createFn: (data: any) => Promise<any>; updateFn: (id: number, data: any) => Promise<any>; deleteFn: (id: number) => Promise<any> }> = {
  news: { 
    title: "News", 
    icon: FileText, 
    fetchFn: fetchPerillaNews,
    createFn: createPerillaNews,
    updateFn: updatePerillaNews,
    deleteFn: deletePerillaNews,
  },
  changelog: { 
    title: "Updates", 
    icon: RefreshCw, 
    fetchFn: fetchPerillaChangelogs,
    createFn: createPerillaChangelog,
    updateFn: updatePerillaChangelog,
    deleteFn: deletePerillaChangelog,
  },
  regions: { 
    title: "Regions", 
    icon: MapPin, 
    fetchFn: fetchPerillaRegions,
    createFn: createPerillaRegion,
    updateFn: updatePerillaRegion,
    deleteFn: deletePerillaRegion,
  },
  varieties: { 
    title: "Varieties", 
    icon: Leaf, 
    fetchFn: fetchPerillaVarieties,
    createFn: createPerillaVariety,
    updateFn: updatePerillaVariety,
    deleteFn: deletePerillaVariety,
  },
  genes: { 
    title: "Genes", 
    icon: Dna, 
    fetchFn: fetchPerillaGenes,
    createFn: createPerillaGene,
    updateFn: updatePerillaGene,
    deleteFn: deletePerillaGene,
  },
  gene_expressions: {
    title: "Gene Expressions",
    icon: Dna,
    fetchFn: fetchPerillaGeneExpressions,
    createFn: createPerillaGeneExpression,
    updateFn: updatePerillaGeneExpression,
    deleteFn: deletePerillaGeneExpression,
  },
  environmental_factors: {
    title: "Environmental Factors",
    icon: Beaker,
    fetchFn: fetchPerillaEnvironmentalFactors,
    createFn: createPerillaEnvironmentalFactor,
    updateFn: updatePerillaEnvironmentalFactor,
    deleteFn: deletePerillaEnvironmentalFactor,
  },
  regional_map_sites: {
    title: "Regional Map Sites",
    icon: MapPin,
    fetchFn: fetchPerillaRegionalMapSites,
    createFn: createPerillaRegionalMapSite,
    updateFn: updatePerillaRegionalMapSite,
    deleteFn: deletePerillaRegionalMapSite,
  },
  regional_environment_values: {
    title: "Regional Environment Values",
    icon: Beaker,
    fetchFn: fetchPerillaRegionalEnvironmentValues,
    createFn: createPerillaRegionalEnvironmentValue,
    updateFn: updatePerillaRegionalEnvironmentValue,
    deleteFn: deletePerillaRegionalEnvironmentValue,
  },
  institutions: { 
    title: "Institutions", 
    icon: Building, 
    fetchFn: fetchPerillaInstitutions,
    createFn: createPerillaInstitution,
    updateFn: updatePerillaInstitution,
    deleteFn: deletePerillaInstitution,
  },
  announcements: { 
    title: "Announcements", 
    icon: Bell, 
    fetchFn: fetchPerillaAnnouncements,
    createFn: createPerillaAnnouncement,
    updateFn: updatePerillaAnnouncement,
    deleteFn: deletePerillaAnnouncement,
  },
  nutrition_data: {
    title: "Nutrition Data",
    icon: Beaker,
    fetchFn: fetchPerillaNutritionData,
    createFn: createPerillaNutritionData,
    updateFn: updatePerillaNutritionData,
    deleteFn: deletePerillaNutritionData,
  },
  downloads: { 
    title: "Downloads", 
    icon: Download, 
    fetchFn: fetchPerillaDownloadFiles,
    createFn: createPerillaDownloadFile,
    updateFn: updatePerillaDownloadFile,
    deleteFn: deletePerillaDownloadFile,
  },
};
  ...
  const fetchForeignKeyData = async () => {
    ...
    try {
      ...
      const [regionsData, varietiesData, genesData, institutionsData] = await Promise.all([
        fetchPerillaRegions(optionParams),
        fetchPerillaVarieties(optionParams),
        fetchPerillaGenes(optionParams),
        fetchPerillaInstitutions(optionParams),
      ]);
              ...
              {(filteredData as NewsData[]).map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    {item.is_published ? <span className="px-2 py-1 text-xs bg-slate-100 text-slate-800 rounded-full">{statusText("published")}</span> : <span className="px-2 py-1 text-xs bg-gray-100 text-gray-800 rounded-full">{statusText("draft")}</span>}
                  </td>
    ...
    <div className="min-h-screen flex">
      ...
      <aside className="w-64 bg-gradient-to-b from-slate-700 to-slate-900 text-white flex-shrink-0 flex flex-col h-screen absolute">
        <div className="p-4 border-b border-slate-600/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-200">{t("admin.panel")}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-600/30">
          <div className="text-sm text-slate-200 mb-2">{t("admin.welcome", { name: currentUser?.username || statusText("admin") })}</div>
