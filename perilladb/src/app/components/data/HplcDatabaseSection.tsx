...
import { fetchPerillaNutritionData, fetchPerillaRegions, fetchPerillaVarieties } from "../../../apis/data_apis";
...
export function HplcDatabaseSection() {
  ...
  useEffect(() => {
    Promise.all([
      fetchPerillaRegions({ limit: 500 }),
      fetchPerillaVarieties({ limit: 500 }),
  ...
  useEffect(() => {
    setLoading(true);
    fetchPerillaNutritionData({ page, pageSize, ...filters })
      .then((data) => {
  ...
  useEffect(() => {
    setPage(1);
    fetchPerillaNutritionData({ limit: 5000, ...filters })
      .then((data) => {
