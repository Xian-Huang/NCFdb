...
import { fetchSesameNutritionData, fetchSesameRegions, fetchSesameVarieties } from "../../../apis/data_apis";
...
export function HplcDatabaseSection() {
  ...
  useEffect(() => {
    Promise.all([
      fetchSesameRegions({ limit: 500 }),
      fetchSesameVarieties({ limit: 500 }),
  ...
  useEffect(() => {
    setLoading(true);
    fetchSesameNutritionData({ page, pageSize, ...filters })
      .then((data) => {
  ...
  useEffect(() => {
    setPage(1);
    fetchSesameNutritionData({ limit: 5000, ...filters })
      .then((data) => {
