...
def build_content(title):
    return f"""{DATABASE} 发布“{title}”相关中文更新。本条内容用于说明{CROP_NAME}数据库在种质资源、木酚素含量、脂肪酸组成、区域环境因子、候选基因和数据整理流程方面的最新进展，帮助用户理解数据来源、适用范围和后续使用方式。
...
用户可根据研究问题选择不同入口。关注营养品质的用户可查看品种、样品批次、油分、芝麻素、芝麻林素和脂肪酸指标；关注分子机制的用户可查看基因注释、候选基因、表达证据和下载文件；关注区域适应性的用户可结合经纬度、温度、降水、光照、土壤和表型记录进行比较。数据库提供证据组织和检索支持，但具体结论仍需结合统计分析和实验验证。
