...
from safflower.models import Announcement, Changelog, News


CROP_NAME = "红花"
DATABASE = "SafflowerDB"
NEWS_IMAGES = [
    "news_images/safflower-genome-release.png",
    "news_images/safflower-nutrition-release.png",
    "news_images/safflower-field-trial.png",
]
NEWS_TOPICS = [
    ("高油分红花材料完成区域试验登记", "高油分材料、籽粒品质和区域试验来源"),
    ("红花功能成分与资源利用会议通知", "功能成分评价、资源利用和数据库使用培训"),
    ("红花参考基因组与通路候选基因更新", "候选基因、通路注释和下载文件"),
]
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        now = timezone.now()
        news_records = list(News.objects.order_by("id")) or [News() for _ in NEWS_TOPICS]
        ...
        for record in changelog_records:
            ...
            record.content = f"{DATABASE} 已切换为中文默认页面，并补充区域环境因子地图、分子指纹图谱框架和中文内容校验规则。"
        ...
        for record in announcement_records:
            ...
            record.content = f"{DATABASE} 将默认展示中文内容。近期活动围绕{CROP_NAME}区域优势品种、环境因子、功能成分和数据库使用开展，英文翻译占位保留，后续可逐步补齐。"
