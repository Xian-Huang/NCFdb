...
const dataTypeConfig: Record<DataType, { title: string; icon: React.ElementType; fetchFn: (params?: any) => Promise<any>; createFn: (data: any) => Promise<any>; updateFn: (id: number, data: any) => Promise<any>; deleteFn: (id: number) => Promise<any> }> = {
  ...
  nutrition_data: {
    ...
    icon: Beaker,
    fetchFn: fetchSafflowerNutritionData,
    createFn: createSafflowerNutritionData,
    updateFn: updateSafflowerNutritionData,
    deleteFn: deleteSafflowerNutritionData,
  },
  downloads: { 
    title: "Downloads", 
    icon: Download, 
  ...
  const fetchForeignKeyData = async () => {
    ...
    try {
      ...
      const [regionsData, varietiesData, genesData, institutionsData] = await Promise.all([
        fetchSafflowerRegions(optionParams),
        fetchSafflowerVarieties(optionParams),
        fetchSafflowerGenes(optionParams),
        fetchSafflowerInstitutions(optionParams),
      ]);
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredData.map((item: any) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="w-28 min-w-28 px-6 py-4 text-right whitespace-nowrap"><button onClick={() => openModal(item)} className="text-red-600 hover:text-red-900 mr-4"><Edit className="h-5 w-5" /></button><button onClick={() => handleDelete(item.id)} className="text-red-600 hover:text-red-900"><Trash2 className="h-5 w-5" /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
