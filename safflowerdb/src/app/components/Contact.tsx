...
export function Contact() {
  ...
  return (
    <div className="space-y-8">
      <section className="border border-red-100 bg-gradient-to-br from-red-50 to-rose-50 p-7 shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-red-700">About & Contact</p>
      ...
      <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <section className="border border-slate-100 bg-white p-6 shadow-sm">
          <h2 className="mb-4 flex items-center gap-2 text-lg font-semibold text-slate-900"><Send className="h-5 w-5 text-red-600" />{t("contact.form")}</h2>
          <form className="space-y-4">
            ...
            <div>
              ...
              <textarea rows={5} className="w-full border border-slate-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-red-500" placeholder={t("contact.message")} />
            </div>
            <button type="submit" className="w-full bg-red-600 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-red-700">
              {t("contact.send")}
            </button>
          </form>
        </section>
        ...
        <div className="space-y-6">
          <section className="border border-slate-100 bg-white p-6 shadow-sm">
            <h2 className="mb-5 flex items-center gap-2 text-lg font-semibold text-slate-900"><Mail className="h-5 w-5 text-red-600" />{t("contact.info")}</h2>
            <div className="grid gap-4 sm:grid-cols-2">
              <Info icon={<MapPin className="h-5 w-5" />} label={t("contact.address")} value="Beijing, China" />
              <Info icon={<Mail className="h-5 w-5" />} label={t("contact.email")} value="contact@safflowerncfdb.org" />
              <Info icon={<Phone className="h-5 w-5" />} label={t("contact.phone")} value="+86 10 1234 5678" />
              <Info icon={<Globe className="h-5 w-5" />} label={t("contact.website")} value="www.safflowerncfdb.org" />
            </div>
          </section>
          ...
          <section className="border border-red-100 bg-white p-6 shadow-sm">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                <h2 className="flex items-center gap-2 text-lg font-semibold text-slate-900"><Users className="h-5 w-5 text-red-600" />{t("contact.collaborators")}</h2>
              ...
              <span className="bg-red-50 px-3 py-1 text-xs font-semibold text-red-700">Safflower Nutrition Data Consortium</span>
            </div>
...
function Field({ label, placeholder, type = "text" }: { label: string; placeholder: string; type?: string }) {
  return (
    <div>
      ...
      <input type={type} className="w-full border border-slate-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-red-500" placeholder={placeholder} />
    </div>
  );
}
