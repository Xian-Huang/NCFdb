...
import { fetchSunflowerNutritionData, fetchSunflowerRegions, fetchSunflowerVarieties } from "../../../apis/data_apis";
...
export function HplcDatabaseSection() {
  ...
  useEffect(() => {
    Promise.all([
      fetchSunflowerRegions({ limit: 500 }),
      fetchSunflowerVarieties({ limit: 500 }),
  ...
  useEffect(() => {
    setLoading(true);
    fetchSunflowerNutritionData({ page, pageSize, ...filters })
      .then((data) => {
  ...
  useEffect(() => {
    setPage(1);
    fetchSunflowerNutritionData({ limit: 5000, ...filters })
      .then((data) => {
