...
def _download_rows(file):
    ...
    rows = [[
        ...
        meta["size"],
        meta["description"],
        meta["downloads"] if meta["downloads"] is not None else "",
    ]]
    return headers, rows
...
class DownloadFileView(APIView):
    def get(self, request, format=None):
        files = DownloadFile.objects.all().order_by('-create_time')
...
class DownloadFileDownloadView(APIView):
    def get(self, request, pk, format=None):
        ...
        if existing_path:
            response = FileResponse(open(existing_path, "rb"), as_attachment=True, filename=os.path.basename(existing_path))
            return response
